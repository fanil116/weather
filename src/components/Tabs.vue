        ...
        <v-col class="d-flex justify-content-around align-center">
          <v-card elevation="0" color="transparent" class="w-100">
            <v-tabs
              v-model="tab"
              align-tabs="start"
              class="tabs rounded-lg"
            >
              <v-tab class="tab tab__first pa-2 pl-4 pr-4" selected-class="tab--selected" :value="'Home'">
                Главная
              </v-tab>
              <v-tab class="tab tab__last pa-2 pl-4 pr-4" selected-class="tab--selected" :value="'Week'">
                Погода за неделю
              </v-tab>
            </v-tabs>
            ...
            <v-tabs-window v-model="tab" class="flex-grow-1">
              <v-tabs-window-item
                ...
                :value="item.tab"
              >
                  <component :is="item.content" class="fill-height" :title="selectedCityName"/>
              </v-tabs-window-item>
            </v-tabs-window>
          </v-card>
